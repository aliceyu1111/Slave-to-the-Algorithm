...
function draw (){
  for (whichHour=0; whichHour<data.Saturday.hours.length; whichHour++){
  
    
    fill (instagramCol);
     stroke (0);
    size = data.Saturday.hours[whichHour].instagram*sizeMultiplier;
    ellipse (random(600, 800), random (150, 300), size, size);
   
  
    fill (messengerCol);
     stroke (0);
    size = data.Saturday.hours[whichHour].messenger*sizeMultiplier; 
    ellipse(random(600, 800), random(150, 300), size, size);
   
    
    fill (snapchatCol);
     stroke (0);
    size = data.Saturday.hours[whichHour].snapchat*sizeMultiplier; 
    rect(random(600, 800), random(150, 300), size, size);
    
    fill (facebookCol);
     stroke (0);
    size = data.Saturday.hours[whichHour].facebook*sizeMultiplier; 
    square (random(600, 800), random(150, 300), size, size);
    
    fill (emailCol);
     stroke (0);
    size = data.Saturday.hours[whichHour].email*sizeMultiplier; 
    ellipse(random(600, 800), random(150, 300), size, size);
  }
}
